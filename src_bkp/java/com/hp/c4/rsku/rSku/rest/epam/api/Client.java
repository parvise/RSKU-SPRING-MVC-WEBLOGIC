 ...
 *
 * This class relies on the correct truststore settings to allow secure
 * connections to be made with the Password Safe.
 *
 * The Password Safe CA Certificate can be exported either using the
 * BeyondInsight Configuration tool for a Software installation or via the UVM
 * console for an Appliance. Please consult the product documentation for
 * details.
 *
 * Once exported the CA certificate should be imported into a Java Keystore
 ...
 * </pre>
 *
 * When Client Certificates are required the following steps must be performed
 * for the API to use a trusted certificate for securing the communication with
 * the Password Safe.
 *
 * The Client Certificate and Private Key can be exported from Password Safe as
 * a PKCS12 file either using the BeyondInsight Configuration tool for a
 * Software installation or via the UVM console for an Appliance. Please consult
...
public class Client {
	...
	private static Logger mLogger = Logger.getLogger(Client.class);
	...
	/**
	 * Creates a new Password Safe API Client connecting to the given host with
	 * custom virtual directory and port
	 *
	 * @param host Password Safe host name or IP Address
	 * @param port Password Safe Port number
	...
	public Client(String host, int port, String dir) throws Client.APIException {
		try {
			...
			System.out.println("syso URL: " + base_url.toString());
			//mLogger.info("info URL: " + base_url.toString());

			CookieHandler.setDefault(new CookieManager(null, CookiePolicy.ACCEPT_ALL));

			headers = new HashMap<String, String>();
		} catch (MalformedURLException me) {
			mLogger.error("Error occured at Client Constructor "+me.getMessage());
	...
	private HttpsURLConnection connect(String path) throws Client.APIException {
		...
		try {
			...
			//System.out.println("syso URL..."+url);
			//mLogger.info("info URL..."+url);
			...
			//System.out.println("syso Connect...."+conn);
			//mLogger.info("info Connect...."+conn);
			return conn;
		} catch (MalformedURLException me) {
			System.out.println("MalformedURLException...."+me.getMessage());
			mLogger.error("Error occured at Client Connect method MalformedURLException.... "+me.getMessage());
			throw new Client.APIException("Invalid Password Safe URL", me);
		} catch (Exception ie) {
			System.out.println("IOException...."+ie.getMessage());
			mLogger.error("Error occured at Client Connect Method "+ie.getMessage());
	...
	private String getResponse(HttpsURLConnection conn) throws Client.APIException {
		try {
			...
			//System.out.println("173  Conn Response...."+readString(conn.getInputStream()));
			//System.out.println("170  Conn Response...."+conn.getURL()+":"+conn.getResponseMessage());
			//mLogger.info("170  Conn Response...."+conn.getURL()+":"+conn.getResponseMessage());
			//System.out.println("171  Conn Response...."+conn.getURL().getPath());
			//mLogger.info("171  Conn Response...."+conn.getURL().getPath());
			//System.out.println("172  Conn Response...."+status);
			//mLogger.info("172  Conn Response...."+status);
			
			
			if (status >= 200 && status < 300) {
				is = conn.getInputStream();
				rv = readString(is);
				//System.out.println("RVV...."+rv);
				//mLogger.info("RVV...."+rv);
			} else {
				switch (status) {
				case 400:
			...
			mLogger.error("Error occured at Client response Method =API Authentication Failed"+ie.getMessage());
			System.out.println("Error occured at Client response Method =API Authentication Failed"+ie.getMessage());
	...
	private Image getResponseImage(HttpsURLConnection conn) throws Client.APIException {
		try {
			...
			mLogger.error("Exception occured at Post Method =Failed to post data to Password Safe"+ie.getMessage());
			System.out.println("Exception occured at Post Method =Failed to post data to Password Safe"+ie.getMessage());
			throw new Client.APIException("Failed to receive response from Password Safe", ie);
		}
	}
	...
	protected String post(String path, String body) throws Client.APIException {
		try {
			HttpsURLConnection conn = connect(path);
			//mLogger.info("info...conn ..: "+conn+":"+path);
			//System.out.println("info...conn ..: "+conn+":"+path);
			//mLogger.info("Body...."+body);
			...
			//mLogger.info("info2...conn ..: "+conn);
			//System.out.println("info2...conn ..: "+conn);
			...
			System.out.println("Exception occured at Post Method =Failed to post data to Password Safe"+ie.getMessage());
			//mLogger.error("Exception occured at Post Method =Failed to post data to Password Safe"+ie.getMessage());
			//mLogger.info("IOException"+ie.getMessage());
	...
	protected String put(String path, String body) throws Client.APIException {
		try {
			...
			mLogger.error("Error occured at Client Put Method "+ie.getMessage());
	...
	private String readString(InputStream is) throws IOException {
		...
		try {
			...
			mLogger.error("Error occured at Client readString Method "+e.getMessage());
			throw e;
		} finally {
			br.close();
		}
	...
	/**
	 * Sign application in to API. API: POST <base>/Auth/SignAppIn
	 *
	 * @param runas A username. The commands will run in the context of the
	 *              specified user.
	...
	public String signAppIn(String runas, String key) throws Client.APIException {
		...
		//System.out.println("syso auth.."+auth);
		//mLogger.info("info auth.."+auth);

		response = post("Auth/SignAppIn", "{}");
		//mLogger.info("info response..."+response);
		//System.out.println("syso response..."+response);
		if (response == null) {
			mLogger.error("Error occured at Client signAppIn Method =API Authentication Failed"+response);
	...
	/**
	 * Search the keystroke events for the given condition The search term can be
	 * any full text search term
	 *
	...
	public String searchKeystrokes(String condition) throws Client.APIException {
		...
		try {
			...
			mLogger.error("EPAM Client searchKeystrokes : "+ ex.getMessage());
		}
	 ...
	 *
	 * @param id       The UUID for the session, this is the Token value from the
	 *                 Session object
	 * @param key      The recording key used to encrypt the log, this is the
	 *                 RecordKey value from the Session object
	 * @param protocol The protocol (currently SSH or RDP), this can be derived from
	 *                 the Protocol value from the Session object (0 = RDP 1 = SSH)
	...
	public String postReplay(String id, String key, String protocol) throws Client.APIException {
		String body = String.format("{\"id\":%s, \"record_key\":%s, \"protocol\":%s, \"headless\":true}",
				jsonString(id), jsonString(key), jsonString(protocol));
	 ...
	 *
	 * @param id The UUID for the session, this is the id value from the Replay
	 *           Session object returned by postReplay
	 * @return JSON object containing the details of the replay session
	 * @throws Client.APIException
	 */
	 ...
	 * @param next   skip the replay session until the output changes by the given %
	 *               (-1 to ignore)
	 * @return JSON object containing the details of the replay session
	 * @throws Client.APIException
	 */
	...
	public String postAsset(String workgroupName, String assetName, String dnsName, String domainName, String ipAddress,
			String macAddress, String assetType) throws APIException {
		String body = formatMessageBody(
				new String[] { "AssetName", "DnsName", "DomainName", "IPAddress", "MacAddress", "AssetType" },
				new String[] { jsonString(assetName), jsonString(dnsName), jsonString(domainName),
						jsonString(ipAddress), jsonString(macAddress), jsonString(assetType) });

		return post("Workgroups/" + workgroupName.replace(" ", "%20") + "/Assets", body);
	}

	public String postManagedSystem(String assetID, String platformID, String netBiosName, String contactEmail,
			String description, String port, String timeout, String passwordRuleID, String dssKeyRuleID,
			String releaseDuration, String maxReleaseDuration, String isaReleaseDuration, String autoManagementFlag,
			String functionalAccountID, String elevationCommand, String checkPassword,
			String changePasswordAfterAnyRelease, String resetPasswordOnMismatch, String changeFrequencyType,
			String changeFrequencyDays, String changeTime) throws APIException {
		String body = formatMessageBody(
				new String[] { "PlatformID", "NetBiosName", "ContactEmail", "Description", "Port", "Timeout",
						"PasswordRuleID", "DSSKeyRuleID", "ReleaseDuration", "MaxReleaseDuration", "ISAReleaseDuration",
						"AutoManagementFlag", "FunctionalAccountID", "ElevationCommand", "CheckPasswordFlag",
						"ChangePasswordAfterAnyReleaseFlag", "ResetPasswordOnMismatchFlag", "ChangeFrequencyType",
						"ChangeFrequencyDays", "ChangeTime" },
				new String[] { platformID, jsonString(netBiosName), jsonString(contactEmail), jsonString(description),
						port, timeout, passwordRuleID, dssKeyRuleID, releaseDuration, maxReleaseDuration,
						isaReleaseDuration, autoManagementFlag, functionalAccountID, jsonString(elevationCommand),
						checkPassword, changePasswordAfterAnyRelease, resetPasswordOnMismatch,
	...
	public String postManagedAccount(String systemID, String accountName, String password, String description,
		...
		String body = formatMessageBody(
				new String[] { "AccountName", "Password", "Description", "PasswordRuleID", "ApiEnabled",
						"ReleaseNotificationEmail", "ChangeServicesFlag", "RestartServicesFlag", "ReleaseDuration",
						"MaxReleaseDuration", "ISAReleaseDuration", "AutoManagementFlag", "CheckPasswordFlag",
						"ResetPasswordOnMismatchFlag", "ChangePasswordAfterAnyReleaseFlag", "ChangeFrequencyType",
						"ChangeFrequencyDays", "ChangeTime" },
				new String[] { jsonString(accountName), jsonString(password), jsonString(description), passwordRuleID,
						apiEnabled, jsonString(releaseNotificationEmail), changeServices, restartServices,
						releaseDuration, maxReleaseDuration, isaReleaseDuration, autoManagement, checkPasswordFlag,
						resetPasswordOnMismatch, changePasswordAfterAnyRelease, jsonString(changeFrequencyType),
						changeFrequencyDays, jsonString(changeTime) });
	...
	public String postUserGroupsLDAP(String groupName, String groupDistinguishedName, String hostName, String bindUser,
			String bindPassword, String port, String useSSL, String membershipAttribute, String accountAtrribute,
			String permissions, String smartRuleAccess) throws APIException {
		String body = formatMessageBody(
				new String[] { "groupType", "groupName", "groupDistinguishedName", "hostName", "bindUser",
						"bindPassword", "port", "useSSL", "membershipAttribute", "accountAttribute", "Permissions",
						"SmartRuleAccess" },
				new String[] { "\"LdapDirectory\"", jsonString(groupName), jsonString(groupDistinguishedName),
						jsonString(hostName), jsonString(bindUser), jsonString(bindPassword), port, useSSL,
						jsonString(membershipAttribute), jsonString(accountAtrribute), permissions, smartRuleAccess });
	...
	public String putCredentials(String accountId, String password, String publicKey, String privateKey,
		...
		String body = formatMessageBody(
				new String[] { "Password", "PublicKey", "PrivateKey", "Passphrase", "UpdateSystem" },
	...
	/**
	 * Takes a list of keys and values and returns a message body that's formatted
	 * in the way it will be posted.
	 * 
	 * @param keys   The keys
